...
def clean_sms_df(df):
    return df.dropna(subset=["sender_id"]) \
             ...
             .withColumn("rating_status",lit("ready")) \
             .withColumn("receiver_cc", substring(col("receiver_id"), 1, 3)) \
             .withColumn("product_code",
                when(col("receiver_cc")=="inc","SMS_NAT")
...
def process_batch(df, epoch_id):
    ...
    # Traitement spécifique par type d'enregistrement
        ...
        if voice_count > 0:
            print("Affichage voice_df(avant l'analyse):")
        ...
        print(f"Voice records: {voice_count}")
        if voice_count > 0:
            print("Affichage voice_df(après l'analyse):")
            voice_df.show(truncate=False)
        # SMS records
        ...
        print(f"sms records: {sms_count}")
        if sms_count > 0:
            print("Affichage sms_df(après l'analyse):")
            sms_df.show(truncate=False)
        # Data records
        ...
        print(f"data records: {data_count}")
        if data_count > 0:
            print("Affichage data_df(après l'analyse):")
            data_df.show(truncate=False)
        ...
        cleaned_batch_df = voice_df \
            ...
            .withColumn("batch_id", lit(epoch_id))  # facultatif, pour traçabilité
        ...
        # 3. (Optionnel) Affichage rapide
        print(f"Batch {epoch_id} écrit : {cleaned_batch_df.count()} enregistrements")
...
try:
    ...
    print("Arrêt du traitement Spark")
    query.stop()